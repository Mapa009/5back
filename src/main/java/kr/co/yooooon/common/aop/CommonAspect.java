package kr.co.yooooon.common.aop;
...
public class CommonAspect {
    ...
    @org.springframework.web.bind.annotation.ExceptionHandler(IdNotFoundException.class)//익셉션 어노테이션. UserNotFoundException.class(우리가 새로 만든 익셉션) 도 선언할 수 있다.
     public void idNotFoundExceptionHandler(HttpServletRequest request, IdNotFoundException e) {
     ...
     public void pwMissMatchException(HttpServletRequest request, PwMissMatchException e) { //패스워드가일치하지않을때나는오류
     ...
     public void dataAccessExceptionHandler(HttpServletRequest request, DataAccessException e) {
        
        ModelAndView mv = new ModelAndView("/errorPage");
        
        mv.addObject("errorCode", -3);
        mv.addObject("errorMsg", e.getMessage());
         System.out.println("#####################DataExceptionHandler###################1");
    ...
    @Around("execution(* kr..sf.*.*(..)) || execution(* kr..applicationService.*.*(..)) || execution(* kr..dao.*.*(..))")
      /*
    ...
    private static final String AOP_TRANSACTION_EXPRESSION = "execution(* kr..sf.*.*(..) ) ";   //포인트컷. 이렇게 설정 값을 상수로 선언함.
